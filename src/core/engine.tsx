...
import Networks, { isMainnetByChainId } from '../utils/network';
...
class Engine {
	...
	refreshTransactionHistory = async (forceCheck: any) => {
		...
		try {
			...
			await AsyncStorage.setItem(LAST_INCOMING_TX_BLOCK_INFO, JSON.stringify(allLastIncomingTxBlocks));
			    ...
			    console.log('Error while fetching all txs', e);
		}
	};
	...
	sync = async ({
		...
		tokens: { allTokens: any, allIgnoredTokens: any },
	}) => {
