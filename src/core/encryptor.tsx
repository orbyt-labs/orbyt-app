...
export default class Encryptor {
	...
	_generateSalt(byteCount = 32) {
		const view: any = new Uint8Array(byteCount);
	...
	_generateKey = (password: any, salt: any, lib: any) =>
		lib === 'original' ? Aes.pbkdf2(password, salt, 5000, 256) : AesForked.pbkdf2(password, salt);

	_keyFromPassword = (password: any, salt: any, lib: any) => this._generateKey(password, salt, lib);

	_encryptWithKey = async (text: any, keyBase64: any) => {
		const iv = await Aes.randomKey(16);
		return Aes.encrypt(text, keyBase64, iv).then((cipher: any) => ({ cipher, iv }));
	};
	 ...
	 */
	encrypt = async (password: any, object: any) => {
