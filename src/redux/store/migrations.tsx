...
import { getAllNetworks, isSafeChainId } from '../../utils/network';
import { toLowerCaseEquals } from '../../utils/general';
...
export const migrations = {
	...
	2: (state: any) => {
		const provider: any = state.engine.backgroundState.NetworkController.provider;
	...
	3: (state: any) => {
		const provider: any = state.engine.backgroundState.NetworkController.provider;
        //@ts-ignore
		const chainId: any = NetworksChainId[provider.type];
		// if chainId === '' is a rpc
		if (chainId) {
	...
	4: (state: any) => {
		...
		const newAllCollectibleContracts: any = {};
		const newAllCollectibles: any = {};
		...
		Object.keys(allTokens).forEach((address) => {
			...
			Object.keys(allTokens[address]).forEach((networkType) => {
                //@ts-ignore
				if (NetworksChainId[networkType] as NetworksChainId) {
                    //@ts-ignore
					newAllTokens[address][NetworksChainId[networkType]] = allTokens[address][networkType];
				} else {
					frequentRpcList.forEach(({ chainId }:{ chainId: any }) => {
	...
	7: (state: any) => {
		const allTokens: any = state.engine.backgroundState.TokensController.allTokens;
		const newAllTokens: any = {};
		if (allTokens) {
		...
		const ignoredTokens: any = state.engine.backgroundState.TokensController.ignoredTokens;
		const newAllIgnoredTokens: any = {};
		Object.keys(allTokens).forEach((accountAddress) => {
			Object.keys(allTokens[accountAddress]).forEach((chainId) => {
